...
import org.apache.spark.sql.functions.{col, concat, hour, mean, lit, second, stddev, to_date, unix_timestamp}
...
object Hello {
  def main(args: Array[String]) = {
    ...
    tripDistanceDf.join(tripDistanceStatsDf)
      .withColumn("isOutlier", detectOutlier(col("trip_distance"), col("UpperLimit"), col("LowerLimit"))).
      filter(col("isOutlier")).orderBy("total_amount").take(10).foreach(println)
